...
topic: Next.js
abstract: A production-focused guide to Next.js 14+ with the App Router — covering Server Components, data fetching patterns, routing conventions, image optimisation, and deployment best practices.
isPublished: true
---
...
Next.js automatically **deduplicates** identical `fetch()` calls within a single render — call `fetch("/api/user")` in ten components, it only makes one request.
...
Use the `metadata` export or `generateMetadata` function — not `<Head>` tags. This integrates with Next.js's streaming architecture.
...
**Route groups** (folders wrapped in parentheses) let you share layouts without affecting the URL — a powerful tool for organising complex apps.
